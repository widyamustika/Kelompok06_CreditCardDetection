...
def show_single():
    ...
    <div class="main-header">
        <h1>🌸 Single Prediction Classifier 🌸</h1>
        <p>Prediksi transaksi kredit dengan model machine learning</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    features = {
        'V3': 0.0, 'V4': 0.0, 'V7': 0.0, 'V10': 0.0, 'V11': 0.0,
        'V12': 0.0, 'V14': 0.0, 'V16': 0.0, 'V17': 0.0, 'V18': 0.0
    }
    ...
    with col1:
        for key in feature_keys[:5]:  # 5 fitur pertama
    ...
    with col2:
        for key in feature_keys[5:]:  # 5 fitur terakhir
            features[key] = st.number_input(
                f"📊 {key}", 
                value=0.0, 
                step=0.01, 
    ...
    with col1:
        use_knn = st.checkbox("🎯 K-Nearest Neighbors", key="single_knn")
        use_svm = st.checkbox("⚡ Support Vector Machine", key="single_svm")
    
    with col2:
        use_nn = st.checkbox("🧠 Neural Network", key="single_nn")
        use_dt = st.checkbox("🌳 Decision Tree", key="single_dt")
    ...
    with col2:
        predict_button = st.button("🔮 **PREDIKSI SEKARANG**", key="single_predict")
    ...
    if predict_button:
        ...
        def predict_and_show(model_path, model_name, emoji):
            try:
                ...
                label = "✅ Transaksi Normal" if pred[0] == 0 else "⚠️ Transaksi Penipuan"
                
                # Styling untuk hasil prediksi
                if pred[0] == 0:
                    st.success(f"{emoji} **{model_name}**: {label}")
                else:
                    st.error(f"{emoji} **{model_name}**: {label}")
